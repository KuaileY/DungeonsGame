...
class MSBuilder
  ...
  def execute
    ...
    output = @sh.execute "#{newBuild_cmd editor_csproj_file}"
  ...
  def csproj_file
    Find.find(@folder) { |f| return f if /\.csproj$/.match(f) != nil && /\/.*\//.match(f) == nil }
  end
  #######################
  def editor_csproj_file
    $fileDir= Dir::pwd
    names=$fileDir.split('/')
    f=""
    for i in 0..names.length-6
      num= names.at(i)
       f<< "#{num}"<<'/'
    end
    require  'find'
    Find.find(f) do |filename|
      #return filename.gsub(".","") if /\.Editor.csproj$/.match(filename) != nil
      return filename if /\.Editor.csproj$/.match(filename) != nil
    end
  end
  ######################
  def build_cmd file
  ...
  def newBuild_cmd file
    "\"#{MSBuilder.ms_build_path}\" \"#{file}\" /property:Configuration=Debug /verbosity:minimal"
  end
...
class NSpecRunner < TestRunner
  ...
  def execute test_name
    ...
    test_dlls.each do |dll|
	  ...
	  output = @sh.execute(test_cmd(dll,""))
