...
public class BuildAnimation : Editor {
    ...
    [MenuItem("Assets/Build Animation")]
    ...
    private static AnimatorController BuildAnimationController(List<AnimationClip> clips, string name)
    {
        ...
        animatorController.AddParameter("EnemyAttack", AnimatorControllerParameterType.Trigger);
        animatorController.AddParameter("EnemyHit", AnimatorControllerParameterType.Trigger);
        ...
        foreach (var state in states)
        {
            ...
            {
                codition.AddCondition(AnimatorConditionMode.If, 0, "EnemyAttack");
                codition.duration = 0;
            }
            if (state.name.Substring(state.name.Length - clipTypes[2].Length, clipTypes[2].Length) == clipTypes[2])
            {
                codition.AddCondition(AnimatorConditionMode.If, 0, "EnemyHit");
                codition.duration = 0;
            }
